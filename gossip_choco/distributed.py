...
#            if self.dist_config['rank']==1: print('recieve', r[0][0]*2.0); break;
    ...
    def _gossip_into_receive_buffer(send_buffer, 
       ...
       # comp_msg = qi = Q(xi-xi_hat) flattened
        #send_flat = flatten_tensors(send_buffer)
        #norm_full = torch.norm(send_flat)
        ...
        #norm_comp = torch.norm(comp_msg) if dist_config['ratio']==0 else torch.norm(comp_msg[0:int(len(comp_msg)/2)])
        #norm_comp=0
        #print(send_flat.size(), comp_msg.size(), norm_full, norm_comp)
        ...
        with gossip_lock:
            data_amt = 0
            #in_msg = sum of q_j's
            in_msg, updated_ps_weight, data_amt = gossiper.mix(comp_msg, send_buffer, gossip_ps_weight, uncompress=uncompress, shapes=shapes, epoch=dist_config['epoch'], iteration=dist_config['iterations'])
        ...
        return updated_ps_weight#, norm_comp
